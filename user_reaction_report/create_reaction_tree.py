def create_reaction_tree(structure, all_users, reaction_dict):
    return _traverse(structure, {}, all_users, reaction_dict)

def _traverse(structure, reaction_tree, all_users, reaction_dict):
    role_id = structure["role_id"]
    users_in_role = _get_users_in_role(all_users, role_id)    
    reactions = []
    ...
    if "children" in structure:
        ...
        for child in structure["children"]:
            node = _traverse(child, {}, all_users, reaction_dict)
            children.append(node)
        reaction_tree["children"] = children
    
    return reaction_tree

def _get_users_in_role(all_users, role_id):
