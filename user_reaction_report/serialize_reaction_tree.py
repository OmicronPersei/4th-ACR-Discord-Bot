...
def serialize_reaction_tree(reaction_tree, emoji_templates, all_roles):
    ...
    return "\n".join(_serialize_nodes(reaction_tree, emoji_dict, roles_dict))

def _serialize_nodes(reaction_tree, emoji_templates, roles_dict):
    buffer = []
    ...
    if "children" in reaction_tree:
        for child in reaction_tree["children"]:
            serialized = _serialize_nodes(child, emoji_templates, roles_dict)
            for s in serialized:
