...
class MockNewMessageDispatcher(NewMessageDispatcher):
...
class TestBase:
    def setUp(self):
        ...
        self.new_message_dispatcher = MockNewMessageDispatcher(self.mock_new_forum_detector, self.mock_discord_service, self.mock_discord_mention_factory, self.mock_forum_thread_data_storage, self.mock_forum_url_thread_factory, self.mock_config, self.mock_clock_signal)

class NewMessageDispatcherTestConstructor(TestCase, TestBase):
    def setUp(self):
        TestBase.setUp(self)
...
class NewMessageDispatcherTestStart(TestCase, TestBase):
    def setUp(self):
        TestBase.setUp(self)
...
class NewMessageDispatcherTestStop(TestCase, TestBase):
    def setUp(self):
        TestBase.setUp(self)
...
class NewMessageDispatcherHandlesNewlyDetectedForumPost(TestCase, TestBase):
    def setUp(self):
        TestBase.setUp(self)
        ...
        self.mock_discord_service.send_channel_message.return_value.set_result(self.mock_sent_message)

        self.mock_discord_service.set_reactions_for_message = MagicMock(return_value=Future())
        self.mock_discord_service.set_reactions_for_message.return_value.set_result(None)
    ...
    async def runTest(self):
        ...
        self.mock_forum_url_thread_factory.get_url.assert_called_with("https://myforum.xyz/", "thread_title")
        ...
        self.mock_discord_service.set_reactions_for_message.assert_called_with(self.mock_sent_message, [ "👍", "👎" ])
